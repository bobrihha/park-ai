...
class Agent:
    ...
    def generate_response(
        ...
        if intent == "birthday" and lead_data:
            ...
            system_prompt += (
                "\n\n💡 ВАЖНОЕ НАПОМИНАНИЕ:"
                "\n- НЕ сообщай о создании заявки после получения телефона"
                "\n- Если клиент задаёт вопрос или сомневается — СНАЧАЛА полностью ответь на его вопрос!"
                "\n- Только ПОСЛЕ ответа мягко подведи к следующему пункту плана"
            ...
            if missing:
                # Указываем ориентир — следующий пункт плана
                next_question = missing[0]
                system_prompt += f"\n\n📌 ОРИЕНТИР (следующий пункт плана): {next_question}"
                if len(missing) > 1:
                    system_prompt += f"\n📋 Ещё не собрано: {', '.join(missing[1:])}"
                system_prompt += "\n\n⚠️ ПОМНИ: Если клиент спрашивает о чём-то — СНАЧАЛА полноценно ответь на его вопрос, а ПОТОМ мягко подведи к этому пункту!"
            else:
